...
public class App {
    ...
    public App(){
        ...
        try {
            server = HttpServer.create(new InetSocketAddress((int) BedrockCloud.getConfig().getDouble("rest-port")), 0);

            Authenticator authenticator = new BasicAuthenticator("cloud") {
                @Override
                public boolean checkCredentials(String username, String password) {
                    return username.equals(BedrockCloud.getConfig().getString("rest-username")) && password.equals(BedrockCloud.getConfig().getString("rest-password"));
                }
            };
            ...
            BedrockCloud.getLogger().info("§aWebserver is running.");
        } catch (IOException e) {
            BedrockCloud.getLogger().error("§cCan't create Webserver.");
        }
    }
}
